...
public class ServerConfigurationKonfigurationsWertePanel extends JPanel {
   ...
   private final JLabel lblCheckpointName;

   private final JLabel lblZoneName;

   private final JLabel lblSuperUserPw;

   private final JLabel lblAdminPw;

   private final JLabel lblIdentityCardPw;

   private final JTextField txtCheckpointName;

   private final JTextField txtZoneName;

   private final JTextField txtSuperUserPw;

   private final JTextField txtAdminPw;

   private final JTextField txtIdentityCardPw;

   private final ServerSetupInformation info;

   private final CheckpointConfigurationCamerasPanel cameraPanel;

   public ServerConfigurationKonfigurationsWertePanel(final ServerSetupInformation info) {
      this.info = info;
      ...
      txtIdentityCardPw.setEnabled(false); // Momentan deaktiviert, das muss man
                                           // in einem grösseren Refactoring
                                           // betrachten.

      cameraPanel = new CheckpointConfigurationCamerasPanel(false, ServiceHelper.getConfigurationsService().getIPCams()
            .getCams());
   ...
   private boolean validateComponent(final JTextField txtField) {
      boolean isValid = true;
      ;
   ...
   public ServerSetupInformation getInfo() {
      ...
      for (final String url : cameraPanel.getUrls().getCams()) {
         info.getIpCamUrls().add(url);
      }
      return info;
   }

   public void applyInfosFromFile(final ServerSetupInformation infoFromFile) {
