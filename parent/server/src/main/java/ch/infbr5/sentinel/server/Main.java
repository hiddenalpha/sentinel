...
public class Main {
   ...
   public static void main(final String[] args) {
      ...
      final InputStream inputStream = Main.class.getResourceAsStream(LOG4J_PROPERTIES_DEV);
      if (inputStream == null) {
      ...
      final DatabaseMigration migration = new DatabaseMigration();
      migration.start();

      // 3. Erst ab hier, darf aktiviert werden, dass Logs in die Datenbank
      // geschrieben werden
      // Falls das vorher schon der Fall ist, wird der EntityManager erstellt,
      // welcher dann einfach die Datenbank anpasst.
      SystemMeldungAppender.ENABLE = true;

      // Comand Line
      readCommandLine(args);

      // 3. UI Erstellen (bereits hier, das UI empf�ngt nun alle Logs)
      if (!headless) {
   ...
   private static void readCommandLine(final String[] args) {
      final CommandLineReader reader = new CommandLineReader(args, ServerConfiguration.IP_ADDRESS,
            ServerConfiguration.PORT, ServerConfiguration.DEBUG_MODE, ServerConfiguration.HEADLESS);
