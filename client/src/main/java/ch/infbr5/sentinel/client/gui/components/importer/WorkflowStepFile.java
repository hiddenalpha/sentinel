...
public class WorkflowStepFile extends WorkflowStep {
   ...
   public WorkflowStepFile(final Frame parent, final WorkflowData data, final WorkflowInterceptor interceptor) {
   ...
   public JPanel getPanel() {
      if (panel == null) {
         ...
         // Extensions
         final StringArray array = ServiceHelper.getPersonenImporterService().getSupportedExtensions();
         ...
         lblInfoBestand = new JLabel(
               "<html>Betrachtet die Personen in der Datei als kompletten Bestand der Truppe. Bestehende Personen im System, welche in der Datei nicht mehr vorhanden sind, werden der Einheit 'ArchivEinheit' zugewiesen.</html>");
         lblInfoEinrueckung = new JLabel(
               "<html>Die Personen in der Datei sind 'nur' Personen, welche einrücken werden. Personen mit unbekannten Einheiten werden der Einheit 'GastEinheit' zugewiesen (Personen, welche einen Gast-WK machen).</html>");
         ...
         btnOpenFile.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(final ActionEvent e) {
               final File file = new FileUpAndDownload(getParent()).showJFileChooser(allowedExtensions);
   ...
   public void finishNext() {
      ...
      sessionKey = ServiceHelper.getPersonenImporterService().initiatImport((new File(currentFilename)).getName(),
            byteData, getData().isKompletterBestand());
   ...
   private void validateDialog() {
      if (currentFilename != null && new File(currentFilename).exists()
            && (einrueckungsImport.isSelected() || bestandedsImport.isSelected())) {
